...
export const BRAND_CATEGORIES: BrandCategory[] = [
  {
    ...
    logo: (
      <svg className="w-12 h-12 sm:w-16 sm:h-16 flex-shrink-0" viewBox="0 0 24 24" fill="none">
...
export default function CategoryGrid({ onSelectSubcategory, selectedSubcategory, productsCount }: CategoryGridProps) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8 w-full" id="brand-category-grid">
      {BRAND_CATEGORIES.map((brand) => {
        ...
        return (
          <div
            ...
            className={`flex items-center justify-between p-6 sm:p-7 bg-white border rounded-2xl sm:rounded-3xl transition-all duration-300 hover:shadow-xl hover:-translate-y-1 cursor-pointer select-none min-h-[120px] sm:min-h-[140px] ${
              isActive 
                ? 'border-blue-600 ring-4 ring-blue-500/15 shadow-lg bg-blue-50/10' 
                : 'border-slate-200/90 shadow-sm hover:border-blue-300'
            }`}
            id={`category-card-${brand.slug}`}
          >
            <div className="flex items-center gap-4 sm:gap-5 min-w-0">
              {/* Logo Frame - Scaled up for 2x visual presentation */}
              <div className="flex items-center justify-center w-14 h-14 sm:w-18 sm:h-18 flex-shrink-0 overflow-hidden bg-slate-50 border border-slate-100 rounded-2xl p-2 shadow-inner">
                {brand.logo}
              </div>

              {/* Text content */}
              <div className="min-w-0">
                <h4 className="text-base sm:text-xl font-black text-slate-900 truncate tracking-tight font-sans leading-tight">
                  {brand.name}
                </h4>
                <p className="text-xs sm:text-sm text-slate-500 font-semibold mt-1 leading-snug">
                  View All Products {count > 0 && `(${count})`}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 pl-3 flex-shrink-0">
              <ChevronRight className={`w-5 h-5 sm:w-6 sm:h-6 transition-transform duration-200 ${
